...
async function excluirAluno(codAluno) {
    ...
    try {
        ...
        if (resposta.ok) {
            ...
            carregarAlunos(); // Atualiza a tabela chamando a listagem novamente
        } else {
            const resultado = await resposta.json();
            alert("Erro ao excluir o aluno: " + (resultado.detail || "Erro desconhecido"));
        }
    } catch (erro) {
        console.error("Erro ao excluir aluno:", erro);
        alert("Não foi possível conectar ao servidor para excluir o aluno.");
    }
}